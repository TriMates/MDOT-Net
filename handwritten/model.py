...
class Degradation2(nn.Module):
    def __init__(self, h_num=100, h_dim=64, n_latents=200):
        super(Degradation2, self).__init__()
        self.dg_encoder_mu = nn.Linear(h_dim, h_dim)
        self.dg_encoder_logvar = nn.Linear(h_dim, h_dim)
        self.w1 = torch.nn.Parameter(torch.FloatTensor(200, h_dim))
        self.w2 = torch.nn.Parameter(torch.FloatTensor(200, h_dim))
        torch.nn.init.xavier_uniform_(self.dg_encoder_mu.weight)
        torch.nn.init.constant_(self.dg_encoder_mu.bias, 0.0)
        torch.nn.init.xavier_uniform_(self.dg_encoder_logvar.weight)
        torch.nn.init.constant_(self.dg_encoder_logvar.bias, 0.0)
    ...
    def get_h(self, z1, z2):
        ...
        h = z.matmul(w)
        h_mean = self.dg_encoder_mu(h)
        h_logvar = self.dg_encoder_logvar(h)
...
class Discrimiter2(nn.Module):
    def __init__(self, n_latents=200):
        super(Discrimiter2, self).__init__()
        self.fc1 = nn.Linear(n_latents, 1)
        torch.nn.init.xavier_uniform_(self.fc1.weight)
        torch.nn.init.constant_(self.fc1.bias, 0.0)
...
class Discrimiter3(nn.Module):
    def __init__(self, n_latents=200):
        super(Discrimiter3, self).__init__()
        self.fc1 = nn.Linear(n_latents, 1)
        torch.nn.init.xavier_uniform_(self.fc1.weight)
        torch.nn.init.constant_(self.fc1.bias, 0.0)
...
class Discrimiter4(nn.Module):
    def __init__(self, n_latents=200):
        super(Discrimiter4, self).__init__()
        self.fc1 = nn.Linear(n_latents, 1)
        torch.nn.init.xavier_uniform_(self.fc1.weight)
        torch.nn.init.constant_(self.fc1.bias, 0.0)
